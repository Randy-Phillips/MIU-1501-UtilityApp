...
NSString * MIU_1501_UtilityApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation MIU_1501_UtilityAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_miu_1501_utilityapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create MIU_1501_UtilityApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "MIU_1501_UtilityApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_miu_1501_utilityapp = [[MIU_1501_UtilityAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_miu_1501_utilityapp];
	
	NSString *_miu_1501_utilityappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _miu_1501_utilityappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_miu_1501_utilityapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:MIU_1501_UtilityApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.miu_1501_utilityapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.miu_1501_utilityapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
